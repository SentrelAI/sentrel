...
export function buildSystemPrompt(
  ...
  if (caps.integrations.enabled && connectedToolkits.length > 0) {
    parts.push(
      ...
      `- **Never use a service's own CLI from Bash in place of its integration.** In particular, ALL GitHub work (issues, PRs, repos, commits, releases) goes through the GITHUB integration tools loaded via search_integrations — NEVER the \`gh\` CLI. \`gh\` is not authenticated in your environment and is blocked; ignore any default guidance telling you to use it.\n\n` +
