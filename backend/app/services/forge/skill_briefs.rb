  ...
  module SkillBriefs
    BATCH_1 = [
      ...
      },
      {
        ...
        notes: "Via the connected-app proxy. Query SOQL for reporting, create/update objects for write-back. Common patterns: log call activity, advance opportunity stage, create task from meeting notes. Respect field-level security — wrap writes in try/catch.",
      ...
      },
      {
        ...
        notes: "Via the connected-app proxy. Patterns: query by filter (status=Active), create page in DB, update properties, add relations. For knowledge-base use, prefer storing structured data in Notion DB vs free-form pages. Pagination — Notion returns 100/page, loop until has_more=false.",
    ...
    BATCH_2 = [
      ...
      { slug: "stripe-billing", name: "Stripe Billing", category: "finance", description: "Customers, subscriptions, invoices, refunds, dispute handling via Stripe API.", notes: "Via the connected-app proxy. Read-heavy by default — writes (refund, void) gated on request_approval. Common: lookup customer by email, list outstanding invoices, advance subscription, issue refund <$50 auto / >$50 approval.", requires_connections: %w[stripe] },
      { slug: "airtable-database", name: "Airtable Database", category: "productivity", description: "CRUD on Airtable bases — records, views, attachments.", notes: "Via the connected-app proxy. Filter by formula, sort, paginate. For attachments, upload via blob then attach URL. Avoid mass deletes — Airtable doesn't have soft-delete.", requires_connections: %w[airtable] },
      { slug: "google-drive-files", name: "Google Drive Files", category: "productivity", description: "Search, read, create, share Drive files. Convert Docs/Sheets to PDF/markdown.", notes: "Via the connected-app proxy. Search by name/owner/modifiedTime. Read Google Docs as markdown via export. Share with role (reader/writer) and audience (email or 'anyone with link'). Watch quota.", requires_connections: %w[google-drive] },
      { slug: "shopify-store", name: "Shopify Store Ops", category: "ops", description: "Orders, products, inventory, customers, fulfillments.", notes: "Via the connected-app proxy. Common: pull today's orders, flag low stock, mark fulfilled, draft customer responses for support tickets via Shopify customer notes.", requires_connections: %w[shopify] },
      ...
      { slug: "github-engineering", name: "GitHub Engineering", category: "engineering", description: "Issues, PRs, releases, actions runs, code search across a repo.", notes: "Via the connected-app proxy + gh CLI fallback. Common: triage new issue → label + assign + draft response; review PR diff and post comments; cut a release with changelog; check failed CI runs.", requires_connections: %w[github] },
      ...
      { slug: "google-sheets", name: "Google Sheets", category: "productivity", description: "Read/write/format Sheets — for reports, trackers, lightweight DBs.", notes: "Via the connected-app proxy. Patterns: append row to log, read range as table, format conditional colors, freeze headers. For analyst handoffs: write a tab + share link.", requires_connections: %w[google-sheets] },
