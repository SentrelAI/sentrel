# Async sibling of TemplatePreviewJob. Runs Forge::SkillPreview in the
# background so the AI Skill Creator UI doesn't sit on a synchronous
# request long enough for the proxy to 504. The page polls
# Admin::SkillsController#new every 2s via the cache token.
#
# State machine in Rails.cache:
#   { status: "queued"|"running"|"done"|"errored", ...timestamps, preview?, error? }
class SkillPreviewJob < ApplicationJob
  queue_as :default

  CACHE_PREFIX = "skill_preview:".freeze
  CACHE_TTL = 1.hour

  def perform(token:, brief:)
    write_state(token, status: "running", started_at: Time.current.iso8601)
    result = Forge::SkillPreview.new(brief: brief.symbolize_keys).call
    ...
    Rails.logger.error "[SkillPreviewJob] #{token} crashed: #{e.class}: #{e.message}"
